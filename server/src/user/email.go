...
func Activation() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		userRow, err := database.Postgres.Query("SELECT email_verified FROM users WHERE email = '" + email + "';")
		defer userRow.Close()
		if err == nil {
			for userRow.Next() {
				userRow.Scan(&verified)
			}
			msg := "OK"
			if !verified {
...
func PasswordReset() func(c *gin.Context) {
	return func(c *gin.Context) {
		// check account activated & send token by mail
		u := user{}
		email := c.Request.URL.Query()["email"][0]
		userRow, err := database.Postgres.Query("SELECT email_verified FROM users WHERE email = '" + email + "';")
		defer userRow.Close()
